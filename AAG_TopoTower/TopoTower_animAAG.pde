...
Param origins[][];
Param angle = new Param(0.11,-2,2);
...
void draw() {
  ...
  mySliders.draw();
  println (frameCount);
  if (frameCount>100 && frameCount<345){
    angle.set(angle.get()-.0013);
  }
  if ( frameCount<360) { saveFrame("TopoTowerHD"+frameCount+".jpg");}
}
...
void makeSpiral2()
{
  ...
  for (int j = 0; j<s.numOfAttractors() ;j++)
  {
    ...
    RotateZ rot = new RotateZ(origin, angle.multiply(( (40*storeyHeight) - 10*storeyHeight)/storeyHeight));
    Pt p = Anar.Pt(0,100,s.alt());
    p.apply(rot);
    spiralPoints[i]=p;
  }
...
{
camera(190,190,160,0,0,110,0,0,-1);
}else if(cam4 == true)
{

Obj createHotelInt(Pts basicLine,Pts basicLineUp,Pts basicLineSlab,Pts basicLineSlabUp){
  Obj ruban = new Obj();
...
Pts hotel2 = new Pts();
 ...
 // hotel.add( curveUniform.pt(0));
  for(int i = 0 ; i<3; i++)
    hotel.add(curveDownGroup.line(0).pt(i));
 hotel.add(curveMidDown.pt(1));
    for(int i = 0 ; i<3; i++)
    hotel.add(extDownGroup.line(0).pt(i));
...
for(int i =51; i<=96; i+=16){
 ...
 for(int i = 0 ; i<3; i++)
    hotel1.add(curveDownGroup.line(6).pt(i));
hotel1.add(mid2.pt(13));
